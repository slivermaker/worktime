...
INSERT INTO DWS_CAPACITY_ALL (
  ...
  SELECT 
    ...
    A.PRODUCT_SORT3,
    A.PRODUCT_SORT4,
    TO_DATE(A.START_DATE,'YYYY/MM/DD'),
    TO_DATE(A.END_DATE,'YYYY/MM/DD'),
    A.ETL_CRT_DT,
    A.ETL_UPD_DT
FROM 
    MDDWD.DWD_CNLXDZB A
RIGHT JOIN 
    MDDWD.DWD_CNJCB B ON (CASE WHEN A.PRODUCTION_LINE ='玫德雅昌-德庆工厂' THEN '玫德雅昌（德庆）'
                            WHEN A.PRODUCTION_LINE ='玫德雅昌-鹤壁工厂' THEN '玫德雅昌（鹤壁）' 
                            ELSE A.PRODUCTION_LINE
                            END)= B.PRODUCTION_LINE AND A.CAPACITY_TYPE = B.CAPACITY_TYPE
CROSS JOIN 
    MDDIM.DIM_MONTH_D C
WHERE 
    C.PERIOD_DATE > DATE '2023-12-01' 
    AND C.PERIOD_DATE < SYSDATE
