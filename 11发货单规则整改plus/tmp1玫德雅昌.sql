DELETE FROM TF_DWD_SALE_DLY  WHERE COMPANY_SALE ='玫德雅昌集团有限公司' ;
COMMIT;
...
,'玫德雅昌集团有限公司' AS COMPANY_SALE                                   --玫德雅昌集团有限公司
,'玫德雅昌'   AS COMPANY_SN                                                 --玫德雅昌  
,'玫德雅昌海外销售部'  AS DEPT1_HIS                                               --玫德雅昌海外销售部
,CASE WHEN G4.ORGNAME LIKE '%销售%办公室%' THEN '玫德雅昌' || G4.ORGNAME ELSE G4.ORGNAME  end DEPT2_HIS                                                      --大区
,CASE WHEN G4.ORGNAME LIKE '%销售%办公室%' THEN '玫德雅昌' || G4.ORGNAME ELSE G4.ORGNAME  end DEPT3_HIS                                                      --科室
,'玫德雅昌海外销售部'  AS DEPT1                                                   --玫德雅昌海外销售部
,CASE WHEN G4.ORGNAME LIKE '%销售%办公室%' THEN '玫德雅昌' || G4.ORGNAME ELSE G4.ORGNAME  end DEPT2                                                      --大区
,CASE WHEN G4.ORGNAME LIKE '%销售%办公室%' THEN '玫德雅昌' || G4.ORGNAME ELSE G4.ORGNAME  end DEPT3                                                      --科室
,REPLACE(G2.FULL_NAME,'库存组织','') AS produce_company                   --生产公司（库存组织）
,'玫德雅昌' PRODUCE_COMPANY_SN ,                                            --玫德雅昌
 NVL(CPFL.REF07,'未归类产品') AS PRODUCT_SORT1,                       --产品大类
 NVL(CPFL.REF08,'未归类产品') AS PRODUCT_SORT2,                       --产品中类
...
--A.STATUS = 'C'--基础信息状态生效C
A.EXPORT_CONFIRM_FLAG = 'Y'--已出库
AND  H.ORDER_TYPE_NAME NOT IN ('工具归还订单', '工具借出订单', '内销免费销售订单', '内销免费样品订单', '内销物资销售免费订单')--订单类型，更新是否销售业绩单据    
AND G1.ORGNAME = '87_玫德雅昌集团2601' --账套，更新销售业绩账套字段-联合条件
AND i2.operatorname NOT IN ('财务制单','陶洁','林鸿杰','朱碧林','李春燕')
AND EMP2.DEPARTMENT_NAME <> '玫德雅昌财务部'
AND G3.ORGNAME = '玫德雅昌海外销售部'
--AND EMP.USERID <> 'liuguoqiang01'*/
;
...
DELETE FROM TF_DWD_SALE_DLY  WHERE COMPANY_SALE ='玫德雅昌（鹤壁）管业有限公司' ;
COMMIT;
