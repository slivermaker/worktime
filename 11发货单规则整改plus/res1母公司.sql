...
,
    DECODE (
        ...
        100 + NVL (
            REPLACE (
                    A.IS_CUSTOMER_UNRELATED,
                    '%',
                    ''
                ),
                13
        )
    ) * 100 AS AMT_M_FACT_OUTTAX --底表字段：无税金额
,
    DECODE (
        H.BUSINESS_TYPE,
        'A',
...
FROM
    ...
    LEFT JOIN ODS_IMS.ODS_CRM_DOMAIN_BASE M ON A.ORG_ID = M.ORG_ID
    AND D.CUSTOMER_CODE = M.CUSTOMER_CODE
    AND M.STATUS = 'C' --领域表
WHERE
