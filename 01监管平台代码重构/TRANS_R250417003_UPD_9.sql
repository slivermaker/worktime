...
SET
    ISSUE_LINE_COUNT = (
        SELECT COUNT(1)
        FROM DWD_R250417003_J01 A
        WHERE
            CHECK_TIME = (
                SELECT MAX(CHECK_TIME)
                FROM DWD_R250417003_J01
            )
            AND A.IS_HX = '否'
    ),
    IS_LAST_EXC = '是',
...
BEGIN
    ...
    IF v_new_data_count = 0 THEN
        ...
        IS_LAST_EXC = '是',  
        JOB_LAST_RUN_TIME = SYSDATE
        WHERE JOB_CODE = 'R250417003_J01';
    ELSE
        UPDATE DWD_DATA_CHECK_JOB 
        SET ISSUE_LINE_COUNT = (
            SELECT COUNT(1) 
            FROM DWD_R250417003_J01 
            WHERE CHECK_TIME = (SELECT MAX(CHECK_TIME) FROM DWD_R250417003_J01) 
              AND IS_HX = '否'
        ),
        IS_LAST_EXC = '是',
        JOB_LAST_RUN_TIME = (SELECT MAX(CHECK_TIME) FROM DWD_R250417003_J01)
        WHERE JOB_CODE = 'R250417003_J01';
    END IF;
    ...
    RULE_CHECK_LAST_TIME = (
        ...
        FROM DWD_DATA_CHECK_JOB 
        WHERE IS_LAST_EXC = '是' 
          AND RULE_CODE = 'R250417003'
    )
    WHERE RULE_CODE = 'R250417003';
END;


SELECT ISSUE_LINE_COUNT,JOB_LAST_RUN_TIME  FROM DWD_DATA_CHECK_JOB
 WHERE JOB_CODE = 'R250417003_J01'