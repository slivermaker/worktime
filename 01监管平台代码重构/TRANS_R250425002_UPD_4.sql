...
SET
    ISSUE_LINE_COUNT = (
        SELECT COUNT(1)
        FROM DWD_R250425002_J01 A
        WHERE
            CHECK_TIME = (
                SELECT MAX(CHECK_TIME)
                FROM DWD_R250425002_J01
            )
            AND A.IS_HX = '否'
    ),
    IS_LAST_EXC = '是',
...
BEGIN
    ...
    IF v_new_data_count = 0 THEN
        ...
        IS_LAST_EXC = '是',  
        JOB_LAST_RUN_TIME = SYSDATE
        WHERE JOB_CODE = 'R250425002_J01';
    ELSE
        UPDATE DWD_DATA_CHECK_JOB 
        SET ISSUE_LINE_COUNT = (
            SELECT COUNT(1) 
            FROM DWD_R250425002_J01 
            WHERE CHECK_TIME = (SELECT MAX(CHECK_TIME) FROM DWD_R250425002_J01) 
              AND IS_HX = '否'
        ),
        IS_LAST_EXC = '是',
        JOB_LAST_RUN_TIME = (SELECT MAX(CHECK_TIME) FROM DWD_R250425002_J01)
        WHERE JOB_CODE = 'R250425002_J01';
    END IF;
