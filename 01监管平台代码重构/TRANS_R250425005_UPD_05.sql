...
UPDATE DWD_DATA_CHECK_JOB 
    SET ISSUE_LINE_COUNT = (
        SELECT  COUNT(1) FROM  DWD_R250425005_J01  A 
        WHERE CHECK_TIME =( SELECT MAX(CHECK_TIME) FROM  DWD_R250425005_J01 ) AND A.IS_HX='否'
        ) 
	,  IS_LAST_EXC =  '是'
    ,JOB_LAST_RUN_TIME = SYSDATE
WHERE JOB_CODE = 'R250425005_J01'
;
...
UPDATE DWD_DATA_CHECK_RULE 
    SET ISSUE_LINE_COUNT = (
        ...
        WHERE   RULE_CODE = 'R250425005'
        ) 
，RULE_CHECK_LAST_TIME =  (
        SELECT  JOB_LAST_RUN_TIME FROM  DWD_DATA_CHECK_JOB   A 
        WHERE   IS_LAST_EXC = '是' AND RULE_CODE = 'R250425005'
        ) 
,RULE_ALERT = (
	SELECT 
	CASE
...
BEGIN
    ...
    ELSE
        ...
        SET ISSUE_LINE_COUNT = (
            SELECT COUNT(1) 
            FROM DWD_R250425005_J01 
            WHERE CHECK_TIME = (SELECT MAX(CHECK_TIME) FROM DWD_R250425005_J01) 
              AND IS_HX = '否'
        ),
        IS_LAST_EXC = '是',
        JOB_LAST_RUN_TIME = (SELECT MAX(CHECK_TIME) FROM DWD_R250425005_J01)
        WHERE JOB_CODE = 'R250425005_J01';
    END IF;
