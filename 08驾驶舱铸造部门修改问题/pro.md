...
DELETE FROM MDADS.ADS_OPPORTUNITY_UNIT
WHERE 
    YPERIOD != LAST_DAY(YPERIOD)
    OR YPERIOD IS NULL;
    

INSERT INTO MDADS.ADS_OPPORTUNITY_UNIT (
    YPERIOD, -- 账期
...
-----------修改非八个组织外的 从二级组织开始 改为其他---------邱治钧 5.12 （郭嘉伟提）
update      MDADS.ADS_OPPORTUNITY_UNIT SET
SALE_REGION_NAME= '其他' ,
FOLLOWER_SALE_COMPANY= '其他',
FOLLOWER_COMPANY= '其他',
FOLLOWER_DEPT= '其他'
WHERE SALE_REGION_NAME  NOT IN ('华北大区','华东大区','华南大区','西南大区','西北大区','国内电力销售部','国内工业能源销售部'，'国内铸造材料销售部');

/*
----------修改重装旅 组织为其他---------邱治钧 5.12 （郭嘉伟提）
UPDATE   MDADS.ADS_OPPORTUNITY_UNIT SET
SALE_REGION_NAME= '其他' ,
FOLLOWER_SALE_COMPANY= '其他',
FOLLOWER_COMPANY= '其他',
FOLLOWER_DEPT= '其他'
WHERE FOLLOWER_SALE_COMPANY  LIKE '%重装旅';
*/

--------------修改四级组织名称为三级组织 邱治钧 5.12 （郭嘉伟提）
UPDATE  MDADS.ADS_OPPORTUNITY_UNIT SET 
FOLLOWER_DEPT=FOLLOWER_SALE_COMPANY
WHERE SALE_REGION_NAME   IN ('国内电力销售部','国内工业能源销售部');


--------------修改是否本区域   邱治钧 5.12 （郭嘉伟提）
UPDATE  MDADS.ADS_OPPORTUNITY_UNIT SET 
FOLLOWER_DEPT=FOLLOWER_SALE_COMPANY，
IS_LOCAL = '是'
WHERE SALE_REGION_NAME   IN ('国内电力销售部','国内工业能源销售部');
...
---补全销售公司  邱治钧
delete  FROM ADS_OPPORTUNITY_UNIT where yperiod is null ;
INSERT INTO ADS_OPPORTUNITY_UNIT
  (

  FOLLOWER_SALE_COMPANY,
   PROVINCE_NAME, -- 省份名称
   CITY_NAME, -- 市名称
   PROJECT_NEED_AMOUNT, -- 项目需求量（万元）
   COMPANY_SUPPLY, -- 我司可供量（万元）
...
SELECT
      ...
      sysdate as  etl_crt_dt,
       sysdate as  etl_upd_dt
    FROM ODS_APD.ODS_APD_SALE_COM_MAPPING A
    LEFT JOIN MDADS.ADS_OPPORTUNITY_UNIT B
      ON A.CITY_NAME = B.CITY_NAME
   WHERE B.CITY_NAME IS NULL;
   

   
   UPDATE ADS_OPPORTUNITY_UNIT tgt
SET
    (tgt.SALE_REGION_NAME, tgt.SALE_PLATFORM_NAME) = (
        SELECT DISTINCT     src.DEPARTMENT_NAME_2, src.DEPARTMENT_NAME_1
        FROM MDDIM.DIM_DEPARTMENT_D src
        WHERE src.DEPARTMENT_NAME_3 = tgt.FOLLOWER_SALE_COMPANY
    )
WHERE EXISTS (
    SELECT 1
    FROM MDDIM.DIM_DEPARTMENT_D src
    WHERE src.DEPARTMENT_NAME_3 = tgt.FOLLOWER_SALE_COMPANY 
    AND TGT.SALE_PLATFORM_NAME is null 
);
